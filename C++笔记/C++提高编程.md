...
* vector(n) ;        //开辟size和capacity均为n的空间
...
* ` remove();`                              //remove的时候只是通过迭代器的指针向前移动来删除，将没有被删除的元素放在链表的前面，并返回一个指向新的超尾值的迭代器。由于remove()函数不是vector成员函数，因此不能调整vector容器的长度。（对vector来说）remove()函数并不是真正的删除，要想真正删除元素则可以使用erase()或者resize()函数。erase()函数可以删除给定区间的元素。它接受两个迭代器参数，这些参数规定了要删除的区间。 
...
2. Insert
3. Delete
    BST的时间复杂度都是O(Logn)，但是哈希表可以达到O(1)，似乎在常见操作中我们都可以使用哈希表，那么什么时候用BST呢？



 这里有几个点很重要：

1. 只需执行BST的有序遍历，就可以按排序顺序获得所有键。这不是哈希表中的常规操作，需要额外的方法。
2. 这样做为了统计，查找最接近更低和更高的元素，BST做范围查询是容易做到。像排序一样，这些操作对于哈希表也不是常规的操作。
3. 与散列相比，BST易于实现，我们可以轻松实现自己的自定义BST。为了实现散列，我们通常依赖于编程语言提供的库。
4. 使用自平衡BST，可以保证所有操作都可以在O（Logn）时间内进行。但是使用散列时，Θ（1）是平均时间，某些特定操作可能会很昂贵，尤其是在调整表大小时。
...
![image-20201008230227554](C:\Users\HZY\AppData\Roaming\Typora\typora-user-images\image-20201008230227554.png)
...
​    当需要修改range中元素，用for(auto& x:range)
   ...
   当vector<bool>返回临时对象，使用auto&会编译错误，临时对象不能绑在non-const l-value reference （左值引用）需使用**auto&&**,初始化右值时也可捕获
...
## *2.*C++ STL mt19937 使用说明**

![image-20201009224841600](C:\Users\HZY\AppData\Roaming\Typora\typora-user-images\image-20201009224841600.png)

```cpp
class Solution {
public:
    double getMinDistSum(vector<vector<int>>& positions) {
        double eps = 1e-7;  // 小于eps出
        double alpha = 1 ;  //学习率
        double decay = 1e-3 ; //衰减率

        int n = positions.size();  //批量梯度下降
        ...
        while(true){
            ...
            for(int i = 0;i<n;i+=batchSize){
                int j = min(n,i+batchSize);
                double dx = 0.0 , dy = 0.0;
                for(int k = i ; k < j ; k++){
                    const auto& pos = positions[k];
                    dx += (x-pos[0]) / (sqrt((pos[0]-x)*(pos[0]-x)+(pos[1]-y)*(pos[1]-y)));
                    dy += (y-pos[1]) / (sqrt((pos[0]-x)*(pos[0]-x)+(pos[1]-y)*(pos[1]-y)));
                }
                x -= alpha*dx;
                y -= alpha*dy;
...
![image-20201010223213422](D:\C++学习\黑马\第5阶段-C++提高编程资料\讲义\image-20201010223213422.png)